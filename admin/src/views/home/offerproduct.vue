<template>
        ...
        <div class="page-header">
          ...
          <div class="page-btn">
            <button
              ...
              @click="dialog = true"
            >
              <img src="/assets/img/icons/plus.svg" alt="img" class="me-1" />Add New Offer Product
            </button>
          </div>
          <v-dialog
          v-model="dialog"
          ...
          <div class="modal-content">
                ...
                <div class="col-lg-12 col-sm-12 col-12">
                    <div class="form-group">
                      ...
                      <model-list-select 
                            :list="$store.getters['category/offercategory']" 
                            option-value="id" option-text="name" 
                            :placeholder="$store.getters['category/offercategory'].length > 0 ? 'Select Category' : 'No data found'"
                            v-model="typedata" />
                    </div>
                  </div>
                  <div class="col-lg-12 col-sm-12 col-12">
  ...
  export default {
    ...
    async created() {
          ...
          this.$store.dispatch("product/getOfferProduct")
          this.$store.dispatch("category/getOfferCategory")
      },
    computed: {
      filteredCategory() {
        const category = this.$store.getters['product/offerProduct'];
              if (this.searchTerm === '') {
                  return category;
              }else{
    ...
    methods:{
          ...
          async saveCategory() {
              ...
              let isSuccess = await this.$store.dispatch('product/saveOfferProduct', this.categoryData);
