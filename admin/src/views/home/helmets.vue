    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title">Add Helmate Product</h5>
                <button
                  @click="dialog = false"
                  class="close"
                >
              ...
              <div class="col-lg-12 col-sm-12 col-12">
                  <div class="form-group">
                    ...
                    <model-list-select 
                          :list="$store.getters['category/helmets']" 
                          option-value="id" option-text="name" 
                          :placeholder="$store.getters['category/helmets'].length > 0 ? 'Select Category' : 'No data found'"
                          v-model="typedata" />
                  </div>
                </div>
                <div class="col-lg-12 col-sm-12 col-12">
                  <div class="form-group">
                    <label>Product </label>
                    <model-list-select 
                          :list="$store.getters['product/helmets']" 
                          option-value="id" option-text="name" 
            ...
            <table class="table table-border">
              ...
              <tbody v-if="paginatedCategorys.length > 0">
                <tr v-for="(item, ind) in paginatedCategorys" :key="ind">
                  ...
                  <td><img style="display:block;width:200px;" :src="`${$store.state.hostName}${item.product.image}`" alt=""></td>
                  <td>
...
export default {
  ...
  watch: {
    ...
    product(data) {
            ...
            this.categoryData.product_id = data.id;
      },
    typedata(data) {
            if(data == undefined) return;
            this.categoryData.type_name = data.name;
      },
  },
  async created() {
        this.$store.dispatch("product/getHelmetProduct")
        this.$store.dispatch("category/getHelmets")
        this.$store.dispatch("product/getHelmateProduct")
    },
  computed: {
    filteredCategory() {
      const category = this.$store.getters['product/helmatsProduct'];
            if (this.searchTerm === '') {
                return category;
            }else{
  ...
  methods:{
        ...
        async saveCategory() {
            if(!this.categoryData.product_id){
                return this.$store.dispatch('notification/error', 'Select Root Category');
            }
            if(!this.categoryData.type_name){
                return this.$store.dispatch('notification/error', 'Please Select Category required');
            }
		    let isSuccess = await this.$store.dispatch('product/saveHelmateProduct', this.categoryData);
            if(isSuccess) {
        ...
        editCategory(item){
            ...
            this.categoryData.type_name= item.name;
            this.product= item.product;
            this.typedata= item;
            this.categoryData.product_id= item.product.id;
        },
