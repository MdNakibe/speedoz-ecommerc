<template>
                ...
                <tbody v-if="paginatedCategorys.length > 0">
                  <tr v-for="(item, ind) in paginatedCategorys" :key="ind">
                    ...
                    <!-- <td>
                      ...
                      <v-icon small @click="approved(item.id,'Return')" color="danger">mdi-cancel</v-icon>
                    </td> -->
                  </tr>
                </tbody>
                <tbody v-else>
  ...
  export default {
    ...
    async created() {
          this.$store.dispatch("aditonal/getOrder", {status: 'Complete'})
      },
    computed: {
      filteredCategory() {
