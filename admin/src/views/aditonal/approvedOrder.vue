<template>
              ...
              <table class="table table-border">
                ...
                <tbody v-if="paginatedCategorys.length > 0">
                  <tr v-for="(item, ind) in paginatedCategorys" :key="ind">
                    ...
                    <td>
                        <v-icon small @click="approved(item.id,'Complete')" color="primary">mdi-checkbox-marked-circle</v-icon>
                        <v-icon small @click="approved(item.id,'Reject')" color="danger">mdi-cancel</v-icon>
                      </td>
                  </tr>
                </tbody>
                <tbody v-else>
  ...
  export default {
    ...
    async created() {
        this.$store.dispatch("aditonal/getOrder", {status: 'Approved'})
      },
    computed: {
      filteredCategory() {
