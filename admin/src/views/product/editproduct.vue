           ...
           <div class="page-header">
            ...
            <div class="col-lg-4 col-sm-4 col-12">
              <div class="form-group">
                ...
                <div class="border p-2 mt-3" v-if="old_preview">
                  <p>Preview Here:</p>
                  <div class="row">
                      <div class="col-lg-12 col-sm-12 col-12">
                        <img :src="`${$store.state.hostName}${old_preview}`" class="img-fluid" />
                    <p class="mb-0">Name {{ old_preview }}</p>
                      </div>
                      <div class="col-lg-4 col-sm-12 col-12">
                        <button type="button" class="btn btn-cancel" @click="resetOld(1)">Clear</button>
                      </div>
                    </div>
                </div>
            ...
            <div class="col-lg-8 col-sm-8 col-8">
              <div class="form-group">
                ...
                <div class="border p-2 mt-3" v-if="old_preview_list.length">
                  <p>Preview Here:</p>
                  <div class="row">
                      <div v-for="item, index in old_preview_list" :key="index" class="col-lg-6 col-sm-16 col-6">
                        <img :src="`${$store.state.hostName}${item}`" class="img-fluid" />
                        <p class="mb-0">file name: {{ item }}</p>
                      </div>
                      <div class="col-lg-4 col-sm-12 col-12">
                        <button type="button" class="btn btn-cancel" @click="resetOld(2)">Clear All</button>
                      </div>
                    </div>
                </div>
...
<script>
  export default {
    data() {
     ...
     computed: {
        productDetails() {
           let data = this.$store.getters['product/edit'];
           return data;
        }
    },
    watch: {
            ...
            handler(data) {
              console.log(data)
                if(data){
                    ...
                    this.productData.regular_price = data.regular_price;
                    this.productData.brand_id = data.brand_id;
                    this.parentcategory = data.category;
                    this.subcategoryData = data.subcategory ? data.subcategory : [];
                    this.chilcategoryData = data.child_category ? data.child_category : [];
                    this.brand = data.brand;
                    this.productData.id = data.id;
                    this.productData.category_id = data.category.id;
                    this.productData.subcategory_id = data.subcategory ? data.subcategory.id : null;
                    this.productData.chilcategory_id = data.child_category ? data.child_category.id : null;
                    this.description = data.description;
                    this.productData.sort_description = data.sort_description;
                    this.old_preview = data.image;
                    this.old_preview = data.image;
                    this.old_image_list = data.related_image ? JSON.parse(data.related_image) : [];
                    this.old_preview_list = data.related_image ? JSON.parse(data.related_image) : [];
                }
            }
        },
      parentcategory(data) {
    ...
    methods: {
      async resetOld(type){
        ...
        await this.$store.dispatch('product/deleteProductImage', data);

        if(type == 1){
          this.productData.single_edit = 1;
          this.old_image = null,
          this.old_preview = null,
          document.getElementById('my-file').value = null;
        }
        if(type == 2){
          this.productData.multi_edit = 1,
          this.old_preview_list = [],
          this.old_image_list = [],
      ...
      let isSuccess = await this.$store.dispatch('product/updateProduct', data);
       if(isSuccess) {
          this.loading = false;
